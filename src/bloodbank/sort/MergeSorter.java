package bloodbank.sort;

import java.util.ArrayList;
import java.util.Comparator;
import java.util.List;

/**
 * Member 5's component.
 * Implements Merge Sort.
 * - Stable (preserves order of equal elements).
 * - Time complexity: O(n log n) in all cases.
 * - Space complexity: O(n) extra space (temporary lists).
 */
public class MergeSorter<T> implements SortStrategy<T> {

    @Override
    public void sort(List<T> list, Comparator<T> comparator) {
        if (list.size() <= 1) return; // already sorted
        // Perform merge sort and overwrite original list
        List<T> sorted = mergeSort(list, comparator);
        for (int i = 0; i < sorted.size(); i++) {
            list.set(i, sorted.get(i));
        }
    }

    // Recursive merge sort
    private List<T> mergeSort(List<T> list, Comparator<T> comparator) {
        if (list.size() <= 1) return list; // base case
        int mid = list.size() / 2;
        // Split list into two halves
        List<T> left = mergeSort(new ArrayList<>(list.subList(0, mid)), comparator);
        List<T> right = mergeSort(new ArrayList<>(list.subList(mid, list.size())), comparator);
        // Merge sorted halves
        return merge(left, right, comparator);
    }

    // Merge two sorted lists into one
    private List<T> merge(List<T> left, List<T> right, Comparator<T> comparator) {
        List<T> result = new ArrayList<>();
        int i = 0, j = 0;
        // Compare elements from both lists and add the smaller one
        while (i < left.size() && j < right.size()) {
            if (comparator.compare(left.get(i), right.get(j)) <= 0) {
                result.add(left.get(i++)); // stable: left element goes first if equal
            } else {
                result.add(right.get(j++));
            }
        }
        // Add remaining elements
        while (i < left.size()) result.add(left.get(i++));
        while (j < right.size()) result.add(right.get(j++));
        return result;
    }

    @Override
    public String getName() {
        return "Merge Sort";
    }
}
